...
import { TIP_KEY_PREFIX, getString, setString } from '@/services/storage.js';
...
export function checkTipVersion(id, timestamp, callback) {
    const storageKey = `${TIP_KEY_PREFIX}${id}`;
    const lastTimestamp = getString(storageKey);

    // 如果时间戳不一致（包括第一次为 null 的情况），则执行回调并更新时间戳
    if (lastTimestamp !== String(timestamp)) {
        callback();
        setString(storageKey, timestamp);
    }
}
