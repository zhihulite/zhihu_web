...
class ZhihuRequest {
    ...
    updateLoginData(loginData, zsts = {}, defaultHeaders = {}) {
        // 传空即停用凭证：服务端登出可能因断网没打成，旧令牌留着等于还在用刚退掉的会话，
        // 清空后首个请求必被判 401，由 401 分支重新引导游客凭证
        const source = loginData?.guest || loginData;
        this.accessToken = source ? "Bearer " + source.access_token : "";

        // 响应自带 cookie 就整组换用，换账号不留旧账号的会话键；不带 cookie 的刷新响应延用上一次的
        const cookieMap = { ...((source && (source.cookie || this.cookieMap)) || {}) };
        if (source?.udid) {
            cookieMap["d_c0"] = source.udid;
        }
    ...
    async request(method, url, data = "", { headers = {}, encryptBody = true, isWWW = false, encryptHead = false, signal = null, noCredentialRebuild = false } = {}) {
        method = method.toUpperCase();
        const isGet = method === 'GET';

        // 每次尝试都现装一份请求头与正文：401 刷新令牌后的重试要按新凭证重算 x-Zse-96 与 Cookie
        const assemble = () => {
            const baseDefaultHeaders = isWWW ? this.commonDefaultHeaders : this.defaultHeaders;
