...
import { KEYS, setString } from '@/services/storage.js'
import { rawHttp } from '@/services/zhihu/transport.js'
...
async function getZstInternal(laesDecryptZst, authorization, udid = '', zst82 = '', zst81 = '') {
  ...
  const json = await rawHttp.post('https://api.zhihu.com/zst/events/s', enc, {
    headers
  })
...
export async function requestGuestCredential() {
  ...
  const data = await rawHttp.post('https://api.zhihu.com/api/account/prod/init/udid_guest', postdata, {
    headers
  })
  ...
  setString(KEYS.udid, data.udid)
  setString(KEYS.zsts, JSON.stringify(zsts))
...
/**
 * 取可用的游客凭证。
 * @param {boolean} force 无条件重新引导（服务端已判定当前凭证不可用时）
 */
export async function ensureGuestCredential(force = false) {
  const current = tokenManager.getLoginData()
  // 游客令牌没有 refresh_token，到期只能重新引导；否则过期后每个请求都 401 且再无出路
  if (current && !force && !tokenManager.isExpired()) return current
